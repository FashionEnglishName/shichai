...
class PurchasesController extends Controller
{
    public function store($id, Request $request){
        ...
        if($article->work_or_tutorial){
            return redirect()->back()->with('warning', '不能为教程添加柴火');
        } else {
    ...
    public function destroy($id){
        ...
        if($article->is_assigned){
            return redirect()->back()->with('warning', "作者已经点燃柴火，无法取回。若一周内作者没发布教程，将自动退还");
        } elseif($article->work_or_tutorial) {
            return redirect()->back()->with('error', '无法从已点燃的柴火堆中取回柴火');
        } else {
            $user = Auth::user();
            $user->refund($id);
